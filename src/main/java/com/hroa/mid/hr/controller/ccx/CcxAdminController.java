...
@Controller
@RequestMapping(value = "wt/ccx/admin")
public class CcxAdminController {
    ...
    public String index(Model model){
        ...
        //获取数据
        model.addAttribute("recordList",ccxMapper.selectList(wrapper));
        return "ccx/ccxAdmin/index";
    }


    @RequestMapping(value = "/ccxpoi", method = RequestMethod.GET)
    public void downloadsExcelDown(HttpServletResponse response) throws IOException {
        Wrapper wrapper = new QueryWrapper();
        ((QueryWrapper) wrapper).orderByDesc("create_date");
        List<Ccx> bgmExcelDownloads = ccxMapper.selectList(wrapper);
      ...
      sheet.addMergedRegion(columnRegion);*/
        ...
        for (int i = 0; i < bgmExcelDownloads.size(); i++) {
            ...
            Ccx bgm = bgmExcelDownloads.get(i);
