...
@Controller
@RequestMapping(value = "wt/niuroufen/admin")
public class YiwanniuroufenAdminController {
    ...
    public String login(User user, Model model){
        if(StringUtils.isNullBoolean(user.getPassword()) || StringUtils.isNullBoolean(user.getUserName())){
            ...
            //验证账号
            return "yiwanniuroufen/yiwanniuroufenAdmin/login";
        }
        Wrapper wrapper = new QueryWrapper();
        ((QueryWrapper) wrapper).setEntity(user);
        if(userMapper.selectList(wrapper).size() > 0){
            model.addAttribute("id","1");
            return "redirect:/wt/niuroufen/admin/index.html";
        }
    ...
    public String index(Model model){
        ...
        //获取数据
        model.addAttribute("recordList",yiwanniuroufenMapper.selectList(wrapper));
        return "yiwanniuroufen/yiwanniuroufenAdmin/index";
    }


    @RequestMapping(value = "/niuroufenpoi", method = RequestMethod.GET)
    public void downloadsExcelDown(HttpServletResponse response) throws IOException {
        Wrapper wrapper = new QueryWrapper();
        ((QueryWrapper) wrapper).orderByDesc("create_date");
        List<Yiwanniuroufen> bgmExcelDownloads = yiwanniuroufenMapper.selectList(wrapper);
      ...
      sheet.addMergedRegion(columnRegion);*/
        ...
        row.createCell(4).setCellValue("留言");//为第三个单元格设值
        //遍历所获取的数据
        for (int i = 0; i < bgmExcelDownloads.size(); i++) {
            row = sheet.createRow(i + 2);
            Yiwanniuroufen bgm = bgmExcelDownloads.get(i);
            ...
            row.createCell(4).setCellValue(bgm.getBeizhu());
        }
